...
#Semester VI, ISMIP1
#This is library open source code available at
            ...
            for obj in all_objs:
                if obj['xmax'] > obj['xmin'] and obj['ymax'] > obj['ymin'] and obj['name'] in self.config['LABELS']:
                    ...
                    if grid_x < self.config['GRID_W'] and grid_y < self.config['GRID_H']:
                        ...
                        y_batch[
                            instance_count, grid_y, grid_x, best_anchor, 5 + obj_indx] = 1  # class probability of the object
...
def check_object_in_grid_anchor_pair(irow, generator_config):
...
idx = 1 #this number is not thoroughly analyzed - could introduce some misbehaviour
images_len = 17125 # how many files are there in F:\GazdaWork\datasets\VOC\VOCdevkit\VOC2012\JPEGImages
l_bound = idx * BATCH_SIZE
r_bound = (idx + 1) * BATCH_SIZE

if r_bound > images_len:
    r_bound = images_len
...
## prepare empty storage space: this will be output
x_batch = np.zeros((r_bound - l_bound, IMAGE_H, IMAGE_W, 3))  # input images
b_batch = np.zeros((r_bound - l_bound, 1, 1, 1, TRUE_BOX_BUFFER,
                    4))  # list of self.config['TRUE_self.config['BOX']_BUFFER'] GT boxes
y_batch = np.zeros((r_bound - l_bound, GRID_H, GRID_W, BOX,
                    4 + 1 + len(LABELS)))  # desired network output
